  ...
  const STATS = [
    { value: 15000, suffix: "+", label: "Transações", icon: Zap },
    { value: 98, suffix: "%", label: "Satisfação", icon: Star },
    { value: 24, suffix: "h", label: "Suporte", icon: Clock },
  ] as const;
  ...
  const AnimatedCounter = ({ end, suffix = "", duration = 1500 }: AnimatedCounterProps) => {
    ...
    return <span ref={ref} className="tabular-nums">{count.toLocaleString("pt-PT")}{suffix}</span>;
  };
  ...
  const Index = () => {
    ...
    // ESTADO DO MENU MULTINÍVEL: "main" | "services" | "info"
              ...
              <Sheet open={sheetOpen} onOpenChange={(open) => { setSheetOpen(open); if(!open) setMenuLevel("main"); }}>
                ...
                <SheetContent side="right" className="w-[320px] bg-white border-l-4 border-amber-400 shadow-2xl flex flex-col justify-between">
                  <div>
                    ...
                    {/* NÍVEL 1: MENU PRINCIPAL (APENAS SERVIÇOS) */}
                    {menuLevel === "main" && (
                    ...
                    {menuLevel === "services" && (
                      <div className="mt-4 flex flex-col gap-2 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        {/* Botão de Informações (Abre Nível 3) */}
                        <Button
                          variant="ghost"
        ...
        <main className="w-full max-w-6xl mx-auto px-4 sm:px-6 -mt-12 space-y-24 pb-20 z-20 relative">
          ...
          <div className="grid grid-cols-3 gap-4">
            {STATS.map((stat, index) => (
              <div key={index} className="bg-white p-6 rounded-2xl border-2 border-amber-200 shadow-lg hover:border-amber-400 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group text-center">
                <div className="w-14 h-14 bg-amber-100 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-amber-400 group-hover:scale-110 transition-all duration-300">
                  <stat.icon size={24} className="text-[#1a4571]" />
                </div>
                <div className="text-3xl font-black text-[#1a4571]"><AnimatedCounter end={stat.value} suffix={stat.suffix} /></div>
