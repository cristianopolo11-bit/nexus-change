...
export default function Index() {
  ...
  // TAXAS FIXAS DA NEXUS (Sem margem de lucro adicionada)
  ...
  // DESBLOQUEIO: Garante que todas as moedas principais usem o WhatsApp
  const isInternationalExchange = useMemo(() => {
    const coreCurrencies = ["USD", "EUR", "AOA", "BRL", "USDT", "USDC"];
      ...
      <div className="w-full max-w-2xl mx-auto space-y-6 animate-in fade-in duration-700 flex flex-col items-center">
        ...
        <header className="w-full flex justify-between items-center py-6 px-2">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/about")} className="text-[10px] font-black uppercase px-4 py-2 hover:text-primary transition-colors">
              Sobre Nós
            </button>
        ...
        <main className="w-full bg-card rounded-[3rem] p-6 md:p-10 border border-foreground/5 shadow-2xl space-y-8 relative overflow-hidden text-center">
          ...
          <div className="pt-8 border-t border-foreground/5 space-y-5">
            <p className="text-[11px] font-black uppercase tracking-[0.25em] text-primary italic">
              {isInternationalExchange ? "Sugestão Internacional" : "Negociar via WhatsApp"}
            </p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {!isInternationalExchange ? (
                <>
                  <button onClick={() => handleWhatsAppAction("COMPRAR")} className="flex flex-col items-center justify-center p-8 rounded-[2.5rem] bg-green-500/10 border border-green-500/20 hover:bg-green-500/20 transition-all group shadow-sm active:scale-95">
                    <TrendingUp className="w-8 h-8 text-green-600 mb-2" />
                    <span className="text-green-700 font-black uppercase italic text-sm">Comprar Agora</span>
                  </button>

                  <button onClick={() => handleWhatsAppAction("VENDER")} className="flex flex-col items-center justify-center p-8 rounded-[2.5rem] bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 transition-all group shadow-sm active:scale-95">
