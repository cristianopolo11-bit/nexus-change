...
const Sell = () => {
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(val) && val >= 0) {
      setAmount(val);
      if (val > 500000) {
        setAmountError("Montante elevado. Confirmação adicional necessária.");
      } else {
        setAmountError(null);
      }
    }
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <div className="w-8 h-8 bg-amber-400 rounded-lg flex items-center justify-center shadow-lg shrink-0">
              <ArrowRightLeft size={16} className="text-[#1a4571]" />
            </div>
      ...
      <main className="max-w-5xl mx-auto px-4 mt-6 sm:mt-8 flex flex-col lg:flex-row gap-6 lg:gap-8 items-start animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
        ...
        <div className="w-full lg:w-[42%] space-y-5 sm:space-y-6 text-left">
          ...
          <form onSubmit={handleFinalizarVenda} id="sell-form" className="bg-white p-5 sm:p-6 rounded-2xl border-2 border-amber-200 shadow-lg space-y-4">
            ...
            <div className="flex items-center gap-2 pt-2 border-t-2 border-amber-100 mb-1">
            ...
            <div className="relative group">
              ...
              <Input
                ...
                className="bg-amber-50/50 h-12 sm:h-14 rounded-xl border-2 border-amber-200 focus-visible:ring-4 focus-visible:ring-amber-300 focus-visible:border-amber-400 transition-all duration-300 text-sm sm:text-base font-bold placeholder:font-normal uppercase"
              />
            ...
            <div className="bg-amber-100/60 rounded-xl p-3 flex items-start gap-2 border border-amber-200">
              ...
              <p className="text-[11px] text-amber-800 font-medium leading-relaxed">
                Os dados são enviados diretamente via WhatsApp. Não guardamos informações de liquidação nos nossos servidores.
              </p>
            </div>
          </form>
        </div>
          ...
          <div className="bg-white rounded-[2rem] border-4 border-amber-400 shadow-2xl p-4 sm:p-6 space-y-5 sm:space-y-6 relative overflow-hidden">
            ...
            <div className="bg-[#1a4571] rounded-2xl p-4 sm:p-6 shadow-2xl border-4 border-amber-400 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-left relative overflow-hidden group">
              ...
              <div className="relative flex flex-col text-white z-10 min-w-0">
                ...
                <div className="text-3xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight mt-1 truncate drop-shadow-md">
                  {isLoading ? (
                ...
                {!isLoading && result > 0 && (
                  <span className="text-[10px] text-amber-300/90 font-medium mt-2 flex items-center gap-1">
                    ...
                    <span className="truncate">Liquidação local direta em conta</span>
                  </span>
                )}
              </div>
            ...
            <div className="flex items-center justify-center gap-3 text-[10px] font-bold text-slate-400 pt-1 flex-wrap">
              <span className="flex items-center gap-0.5 text-slate-500"><ShieldCheck size={12} className="text-emerald-500" /> SSL Protegido</span>
              <span className="w-1 h-1 bg-slate-300 rounded-full" />
              <span className="flex items-center gap-0.5 text-slate-500"><Clock size={12} className="text-amber-500" /> Operação Direta</span>
              <span className="w-1 h-1 bg-slate-300 rounded-full" />
              <span className="flex items-center gap-0.5 text-slate-500"><Zap size={12} className="text-amber-500" /> Liquidação Instantânea</span>
            </div>
          </div>
        </div>
      </main>
